...
This folder does not build images. It expects the backend and frontend
images to already exist as OCI-archive tarballs in [`images/`](images/)
(e.g. produced elsewhere with `docker buildx build --output type=oci,dest=...`
or `podman save --format oci-archive`) and just pushes and deploys them.
...
deployment/
  ...
  images/                pre-built OCI-archive tarballs (backend + frontend)
  push-images.sh          pushes images/*.tar.gz to REGISTRY (skopeo, no build)
...
**Phase 1 — push.** Run this wherever `skopeo` is installed and already
authenticated to your registry (`docker login`, or for ECR:
`aws ecr get-login-password --region <region> | docker login --username AWS
--password-stdin <account>.dkr.ecr.<region>.amazonaws.com` — skopeo reads
the same credentials `docker login` writes to `~/.docker/config.json`).
`push-images.sh` reads the tarballs straight out of `images/` — it doesn't
build anything and doesn't need a Docker daemon.
...
if you'd rather keep it minimal (the `images/` tarballs and Dockerfiles
aren't needed here — only phase 1 touches them) — carrying over the same
`.env` (or at least the same `REGISTRY`/`IMAGE_TAG`) from phase 1.
...
Drop the new OCI-archive tarball(s) in `images/` (overwriting the old ones,
or point `BACKEND_IMAGE_TAR`/`FRONTEND_IMAGE_TAR` at wherever they landed),
then:

```bash
cd deployment
./push-images.sh            # phase 1, wherever skopeo + registry auth live
